...
LONDON_COORDINATES = {"latitude": 51.3, "longitude": -0.12}
BERMINGHAM_COORDINATES = {"latitude": 52.8, "longitude": -1.12}
...
# Environement variables for configuration
...
def delivery_report(err, msg):
    if err is not None:
        print(f'Message deliveryt failed : {err}')
    else:
...
def simulate_journey(producer, device_id):
    while True:
        ...
        if ( vehicle_data['location'][0] >= BERMINGHAM_COORDINATES['latitude']
                and vehicle_data['location'][1] >= BERMINGHAM_COORDINATES['lonhitude']):
...
if __name__ == '__main__':
    ...
    try:
        simulate_journey(producer, 'Vehicle-Hank-123')
